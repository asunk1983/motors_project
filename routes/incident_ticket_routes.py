...
#
# Открыто всем ролям на чтение/создание/редактирование — общий гейт для
# не-reader на запись уже обеспечен auth_bp.before_app_request, точечных
# проверок роли здесь нет, КРОМЕ удаления заявки (ТЗ раздел 2.1.4 —
# только superadmin, см. ниже).
#
# created_by_user_id берётся из request.current_user, а не из тела
# запроса (иначе кто угодно мог бы создать заявку от чужого имени,
# подставив другой id в JSON).
#
