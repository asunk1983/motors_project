...
function switchTab(tabId) {
    ...
    if (tabId === 'catalog') {
        ...
        if (tabId === 'audit' && typeof loadAuditTab === 'function') loadAuditTab();
    }
}
...
const ENGINE_COLUMNS = [
    ...
    { key: 'status', label: 'Статус', sortField: 'status', defaultVisible: true,
      thClass: 'col-engine-status sortable',
    ...
    { key: 'last_edited_by', label: 'Изменил', sortField: 'last_edited_by', defaultVisible: true,
      thClass: 'sortable',
      render: e => escapeHtml(e.last_edited_by || '—') },
    ...
    { key: 'workshop', label: 'Цех', sortField: 'workshop', defaultVisible: false,
      thClass: 'sortable',
    ...
    { key: 'bearing_front', label: 'Подшипник передний', sortField: 'bearing_front', defaultVisible: false,
      thClass: 'sortable',
      render: e => highlightMatch(e.bearing_front, currentSearchQuery) || '—' },
    { key: 'bearing_rear', label: 'Подшипник задний', sortField: 'bearing_rear', defaultVisible: false,
      thClass: 'sortable',
      render: e => highlightMatch(e.bearing_rear, currentSearchQuery) || '—' },
    { key: 'shaft_diameter', label: 'Диаметр вала (мм)', sortField: 'shaft_diameter', defaultVisible: false,
      thClass: 'mono sortable',
      render: e => highlightMatch(e.shaft_diameter, currentSearchQuery) || '—' },
    { key: 'protection_class', label: 'Степень защиты', sortField: 'protection_class', defaultVisible: false,
      thClass: 'sortable',
      render: e => highlightMatch(e.protection_class, currentSearchQuery) || '—' },
    { key: 'mounting_type', label: 'Тип крепления', sortField: 'mounting_type', defaultVisible: false,
      thClass: 'sortable',
      render: e => highlightMatch(e.mounting_type, currentSearchQuery) || '—' },
    { key: 'temp_sensor', label: 'Датчик температуры', sortField: 'temp_sensor', defaultVisible: false,
      thClass: 'sortable',
      render: e => highlightMatch(e.temp_sensor, currentSearchQuery) || '—' },
    { key: 'encoder', label: 'Энкодер', sortField: 'encoder', defaultVisible: false,
      thClass: 'sortable',
      render: e => highlightMatch(e.encoder, currentSearchQuery) || '—' },
    { key: 'cooling', label: 'Охлаждение', sortField: 'cooling', defaultVisible: false,
      thClass: 'sortable',
      render: e => highlightMatch(e.cooling, currentSearchQuery) || '—' },
    { key: 'note', label: 'Примечание', sortField: 'note', defaultVisible: false,
      thClass: 'col-note sortable',
      render: e => {
          const val = e.note || '';
          if (!val) return '—';
    ...
    // Отображение — все значения режимов через запятую (GROUP_CONCAT,
    // см. repositories/engine_repo.py::get_all). Сортировка — по MIN
    // среди режимов данного двигателя (backend отдаёт готовый ключ
    // modes_*_sort, здесь только имя поля sortField для sortTable()).
    { key: 'modes_frequency', label: 'Частоты (Гц)', sortField: 'modes_frequency', defaultVisible: false,
      thClass: 'mono sortable',
      render: e => e.modes_frequency || '—' },
    { key: 'modes_power', label: 'Мощности (кВт)', sortField: 'modes_power', defaultVisible: false,
      thClass: 'mono sortable',
      render: e => e.modes_power || '—' },
    { key: 'modes_voltage', label: 'Напряжения (В)', sortField: 'modes_voltage', defaultVisible: false,
      thClass: 'mono sortable',
      render: e => e.modes_voltage || '—' },
    { key: 'modes_connection_type', label: 'Типы подключения', sortField: 'modes_connection_type', defaultVisible: false,
      thClass: 'sortable',
      render: e => e.modes_connection_type || '—' },
    { key: 'modes_current', label: 'Токи (А)', sortField: 'modes_current', defaultVisible: false,
      thClass: 'mono sortable',
      render: e => e.modes_current || '—' },
    { key: 'modes_rpm', label: 'Обороты (об/мин)', sortField: 'modes_rpm', defaultVisible: false,
      thClass: 'mono sortable',
      render: e => e.modes_rpm || '—' },
];
