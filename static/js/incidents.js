...
// Отдельный, независимый модуль: свободная запись "что/где/кто/как
...
function switchIncidentsSubtab(name) {
    ...
    if (name === 'journal') {
        // Боковое дерево мест на подвкладке "Журнал" (incidentLocationTree.js)
        // держит свой снэпшот /api/locations в памяти. loadIncidentsTab()
        // грузит его при ПЕРВОМ заходе на вкладку "Инциденты" или при
        // возврате с другой ВЕРХНЕЙ вкладки, но при переключении подвкладок
        // внутри "Инцидентов" (Журнал ↔ Справочники) никакого обновления
        // не было — после правки справочника мест (Справочники) дерево
        // журнала оставалось со старым списком до F5. Поэтому при каждом
        // заходе на "Журнал" заново вызываем загрузку.
        if (typeof loadIncidentLocationTree === 'function') loadIncidentLocationTree();
    }
}
