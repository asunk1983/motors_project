...
async function apiFetch(url, options = {}) {
    ...
    if (resp.status === 401) {
        // Токен недействителен/отозван (истёк или админ сбросил сессию,
        // пока пользователь работал с открытой карточкой) — чистим и
        // ПОЛНОСТЬЮ ПЕРЕЗАГРУЖАЕМ страницу, а не просто накладываем
        // логин-экран поверх текущего DOM. showLoginScreen() раньше
        // оставлял активными все открытые модалки, несохранённые формы,
        // выбранные чекбоксы экспорта и т.п. "под" оверлеем — тот же
        // класс проблемы, из-за которого restoreServerBackup() в
        // backupManager.js уже использует location.reload() после
        // восстановления бэкапа. #appLoadingOverlay статичен в
        // index.html и снова покажется при перезагрузке, затем его
        // скроет hideAppLoadingOverlay() из свежего authInit().
        clearAuth();
        location.reload();
...
async function logout() {
    ...
    clearAuth();
    // Полная перезагрузка страницы вместо showLoginScreen(): логин-экран
    // раньше просто накладывался поверх текущего DOM, а всё состояние
    // сессии (открытая карточка двигателя, несохранённая форма, выбранные
    // для экспорта чекбоксы, activeWorkshop/activeLocation и т.д.)
    // оставалось активным "под" оверлеем — именно это и создавало эффект
    // "вышел из аккаунта, но активное окно осталось". #appLoadingOverlay
    // в index.html статичен и покажется заново при перезагрузке, затем
    // его скроет hideAppLoadingOverlay() из authInit() на чистой странице.
    location.reload();
}
